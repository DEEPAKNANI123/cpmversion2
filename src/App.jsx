...
function canEdit() {
  ...
  return day <= 5; // Configurable: First 5 days of month
}
...
function Employee({ profile, activeUser, showToast }) {
  ...
  async function fetchMonthlyEvidence() {
    ...
    if (data && !error) {
      setMonthlyEvidence({
        achievements: data.emp_achievements || "",
        blockers: data.emp_blockers || "",
        learning: data.emp_learning || "",
        proofPoints: data.emp_proof_points || "",
        attachments: data.emp_attachments || [],
        rating_status: data.rating_status || "PENDING",
  ...
  async function handleEvidenceSubmit() {
    ...
    const evidenceData = {
      ...
      attachments: monthlyEvidence.attachments,
      rating_status: 'PENDING'
    };

    console.log("📤 SUBMITTING EVIDENCE DATA:", evidenceData);

    const { error } = await supabase.from('monthly_reviews').upsert(evidenceData, { onConflict: 'employee_id,cycle_id' });

    if (!error) {
      showToast('✓ Monthly evidence submitted and saved', 'var(--green)');
      // Clear the inputs as requested by the user
      setMonthlyEvidence({
        achievements: "",
        blockers: "",
        learning: "",
  ...
  async function handleSubthemeAction(themeId, action) {
    ...
    const { error } = await supabase.from('employee_subtheme_alignment').update({ status: statusMap[action] }).eq('id', themeId);
  ...
  async function handleSubthemeSubmit(parentThemeId, formData, targetEmployeeId = null, existingId = null) {
    ...
    const dateRange = `${formatDate(formData.start_date || today)} - ${formatDate(formData.end_date || today)}`;
    const subDate = formatDate(new Date());
  ...
  return (
    <div className="page">
      ...
      <div className="page-sub" style={{ background: 'var(--bg2)', padding: '10px 20px', borderRadius: 12, marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid var(--border)' }}>
        <div className="v-stack">
          <div style={{ fontSize: 13, fontWeight: 700 }}>{profile?.first_name} {profile?.last_name} · {profile?.employee_id}</div>
          <div style={{ fontSize: 11, opacity: 0.6 }}>{profile?.job_title} · April 2026 Cycle</div>
        </div>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <div className="v-stack" style={{ alignItems: 'flex-end' }}>
            <div style={{ fontSize: 9, fontWeight: 800, color: 'var(--cyan)' }}>DATA SYNC STATUS</div>
            <div style={{ fontSize: 11, fontWeight: 600 }}>{themes.length} Execution Items Found</div>
          </div>
          <button className="badge badge-gray" onClick={refreshEmployeeDash} style={{ cursor: 'pointer', padding: '4px 10px', border: '1px solid var(--cyan)', color: 'var(--cyan)' }}>↻ Sync Now</button>
        </div>
      </div>
      ...
      <div className="frame" style={{ borderLeft: '4px solid var(--cyan)', background: '#fff', marginTop: 12, padding: '12px 20px', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            {(profile?.role === 'manager' || profile?.role === 'hod' || profile?.role === 'hr') && (
              <button
                className="btn-link"
                          ...
                          <div style={{ display: 'flex', gap: 20, paddingLeft: 0, marginTop: 4 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text3)' }}>
                              <span style={{ fontSize: 14 }}>○</span> Period: {
                                (sub.description || sub.global_subthemes?.description)?.includes('|')
                                ? (sub.description || sub.global_subthemes?.description).split('[')[1].split('|')[0].replace(/-/g, '→')
                                : 'Select Period →'
                              }
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text3)' }}>
                              <span style={{ fontSize: 14 }}>○</span> Submitted {
                                (sub.description || sub.global_subthemes?.description)?.includes('|')
                                ? (sub.description || sub.global_subthemes?.description).split('|')[1].split(']')[0]
                                : 'Just now'
                              }
                            </div>
                          </div>
      ...
      {profile?.role === 'manager' && (
        <div className="frame" style={{ borderLeft: '4px solid var(--cyan)', background: '#fff', marginTop: 40, padding: 24, borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
            ...
            {team.filter(m => m.id !== activeUser).map(m => {
              ...
              return (
                <div key={m.id} style={{ background: '#fff', borderRadius: 12, border: '1px solid var(--border)', padding: 0, overflow: 'hidden' }}>
                  {/* USER INFO BAR (IMAGE 2) */}
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 16px', background: '#fff', borderBottom: '1px solid var(--border)' }}>
                    <div className="emp-avatar" style={{ width: 32, height: 32, fontSize: 11, background: 'var(--cyan)' }}>{getInitials(m)}</div>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 13, color: '#1a1a1a' }}>{m.first_name} {m.last_name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 500 }}>{m.job_title}</div>
                    </div>
                  </div>

                  <div style={{ padding: '12px 16px', background: '#f8fafc' }}>
                    {rootThemes.filter(rt => reportsThemes.some(t => t.parent_id === rt.id || t.main_theme_id === rt.id)).map(rt => {
                      const sub = reportsThemes.find(st => (st.parent_id === rt.id || st.main_theme_id === rt.id) && st.employee_id === m.id && st.assigned_by === m.id);
                      const managerSupplement = reportsThemes.find(st => (st.parent_id === rt.id || st.main_theme_id === rt.id) && st.employee_id === m.id && st.assigned_by === activeUser);

                      return (
                        <div key={rt.id} style={{ background: '#eef2f6', borderRadius: 10, border: '1px solid var(--border)', marginBottom: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                          <div style={{ fontSize: 9, fontWeight: 900, color: 'var(--purple)', padding: '8px 12px', background: '#fff', borderBottom: '1px solid var(--border)', textTransform: 'uppercase', letterSpacing: 0.5 }}>ROOT: {rt.title || rt.name}</div>

                          <div style={{ display: 'flex', gap: 0, background: '#f1f5f9' }}>
                            {/* EMPLOYEE EXECUTION (CYAN BAR) */}
                            <div style={{ flex: 1, padding: '12px', borderRight: '1px solid var(--border)' }}>
                              <div style={{ fontSize: 8, fontWeight: 800, color: 'var(--cyan)', borderLeft: '3px solid var(--cyan)', paddingLeft: 8, marginBottom: 6, textTransform: 'uppercase' }}>EMPLOYEE EXECUTION</div>
                              {sub ? (
                                <>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>{sub.title}</div>
                                  <div style={{ fontSize: 11, color: '#475569', marginTop: 4, lineHeight: 1.4 }}>{sub.description}</div>
                                </>
                              ) : (
                                <div style={{ fontSize: 11, color: '#94a3b8', fontStyle: 'italic' }}>No execution item added.</div>
                              )}
                            </div>

                            {/* SUPERIOR ALIGNMENT (TEAL BAR) */}
                            <div style={{ flex: 1, padding: '12px' }}>
                              <div style={{ fontSize: 8, fontWeight: 800, color: '#0f766e', borderLeft: '3px solid #0f766e', paddingLeft: 8, marginBottom: 6, textTransform: 'uppercase' }}>SUPERIOR STRATEGIC ALIGNMENT</div>
                              {managerSupplement ? (
                                <>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>{managerSupplement.title}</div>
                                  <div style={{ fontSize: 11, color: '#475569', marginTop: 4, lineHeight: 1.4 }}>{managerSupplement.description}</div>
                                </>
                              ) : (
                                <button
                                  className="btn-link"
                                  style={{ color: '#64748b', fontSize: 10, fontWeight: 700, border: '1px solid #cbd5e1', padding: '6px 12px', borderRadius: 4, background: '#fff', cursor: 'pointer', transition: 'all 0.2s' }}
                                  onMouseOver={e => e.currentTarget.style.borderColor = 'var(--cyan)'}
                                  onMouseOut={e => e.currentTarget.style.borderColor = '#cbd5e1'}
                                  onClick={() => {
                                    setActiveSubthemeId(rt.id);
                                    setNewDirective({
                                      title: `Verification: ${rt.title || rt.name || "Theme"}`,
                                      description: `Validated execution for ${m.first_name}. Outcomes met core pillars.`,
                                      category: rt.category || "Business Impact",
                                      start_date: today,
                                      end_date: today
                                    });
                                  }}
                          ...
                          <div className="h-stack" style={{ gap: 10, padding: '8px 16px', background: '#fff', borderTop: '1px solid var(--border)', justifyContent: 'flex-end' }}>
                            <button 
                              className="btn-outline" 
                              style={{ border: '1px solid #e2e8f0', color: 'var(--green)', fontSize: 11, padding: '6px 16px', borderRadius: 6, background: '#fff', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }} 
                              onClick={() => handleSubthemeAction(sub?.id, 'approve')}
                            >
                              ✓ Verify Branch
                            </button>
                            <button 
                              className="btn-primary" 
                              style={{ background: '#f1f5f9', color: '#94a3b8', border: '1px solid #e2e8f0', fontSize: 11, padding: '6px 16px', borderRadius: 6, cursor: 'not-allowed', fontWeight: 600 }}
                            >
                              Package & Submit to HR →
                            </button>
                          </div>
                        </div>
                      );
          ...
          <div className="v-stack" style={{ gap: 12 }}>
            <div style={{ padding: '8px 12px', background: 'var(--bg2)', borderRadius: 10, border: '1px solid var(--frame-border)', marginBottom: 8 }}>
              ...
              <select
                ...
                style={{ background: '#fff', cursor: 'pointer', padding: '6px 12px', fontSize: 11, width: '100%', border: '1px solid var(--border)', borderRadius: 6 }}
              ...
              >
                ...
                {themes.map(t => (
                  <option key={t.id} value={t.id}>{t.title || t.global_subthemes?.title}</option>
                ))}
              </select>
            </div>
              ...
              <div className="v-stack" style={{ gap: 8, padding: '12px', background: '#fff', borderRadius: 8, border: '1px solid var(--border)' }}>
                {monthlyEvidence.attachments.map((file, idx) => (
                  <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button
                      style={{ background: 'none', border: 'none', color: 'var(--red)', fontSize: 10, cursor: 'pointer', fontWeight: 700 }}
      ...
      <div className="frame" style={{ borderLeft: '4px solid var(--cyan)', background: '#fff', marginTop: 16, padding: '12px 16px', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
        ...
        <table className="hist-table-v3" style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {reviews.filter(r => r.employee_id === activeUser).length > 0 ? (
              reviews.filter(r => r.employee_id === activeUser).sort((a,b) => new Date(b.submitted_at || 0) - new Date(a.submitted_at || 0)).map((row, idx) => (
                <tr key={idx} style={{ borderTop: '1px solid var(--bg3)' }}>
                  <td style={{ padding: '10px 0', fontWeight: 700, fontSize: 12, color: '#334155' }}>
                    {row.cycle_id === 'ffffffff-ffff-ffff-ffff-ffffffffffff' ? 'April 2026' : row.cycle_id}
                  </td>
      ...
      <div className="action-row" style={{ display: 'flex', gap: 16, marginTop: 32, alignItems: 'center', paddingBottom: 60 }}>
        ...
        <button
          className="btn-primary"
          style={{ padding: '12px 32px', background: 'var(--cyan)', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 13 }}
          onClick={async () => {
...
function ManagerPortal({ profile, activeUser, showToast }) {
  ...
  async function handleReviewSubmit(employeeId) {
    const themeValues = Object.values(binaryInputs.themeResults);
    const yesCount = themeValues.filter(v => v === 'YES').length;
    const neutralCount = themeValues.filter(v => v === 'NEUTRAL').length;
    
    let overall = 'NO';
    if (yesCount >= 2) overall = 'YES';
    else if (yesCount + neutralCount >= 2) overall = 'NEUTRAL';
    ...
    const { error } = await supabase.from('monthly_reviews').upsert(reviewRecord, { onConflict: 'employee_id,cycle_id' });
  ...
  async function handleThemeAction(themeId, action) {
    ...
    const alignStatusMap = { 'approve': 'APPROVED', 'return': 'PENDING', 'reject': 'REJECTED' }; // Mapping for alignment table
    const colorMap = { 'approve': 'var(--green)', 'return': 'var(--yellow)', 'reject': 'var(--red)' };

    // Find the theme in state to see if it's an alignment record
    const theme = teamThemes.find(t => t.id === themeId);
    const superiorInput = theme?.temp_superior_input || theme?.superior_input || "";
    ...
    if (!isAlignment) updatePayload.superior_input = superiorInput;
  ...
  return (
    <div className="page" style={{ paddingBottom: 60 }}>
      ...
      <div className="h-stack" style={{ gap: 12, marginBottom: 24 }}>
        <button className="badge badge-blue" onClick={() => fetchManagerData()}>↻ Refresh Team</button>
      ...
      <div className="frame" style={{ border: '1px solid var(--orange)', background: 'rgba(188,76,0,0.02)' }}>
                              ...
                              <div key={t.id} style={{ padding: '12px 16px', background: '#fff', borderRadius: 8, border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: 6 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                  ...
                                  <div className="validation-actions" style={{ display: 'flex', gap: 6 }}>
                                    ...
                                    <button className="badge badge-yellow" style={{ cursor: 'pointer', border: 'none' }} onClick={() => handleThemeAction(t.id, 'return')}>↩ Return</button>
                                ...
                                <div style={{ display: 'flex', gap: 16, marginTop: 4 }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: 'var(--text3)' }}>
                                    <span>○</span> Period: {t.description?.includes('|') ? t.description.split('[')[1].split('|')[0].replace(/-/g, '→') : '01/04/2026 → 30/04/2026'}
                                  </div>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: 'var(--text3)' }}>
                                    <span>○</span> Submitted {t.description?.includes('|') ? t.description.split('|')[1].split(']')[0] : formatDate(t.submitted_at || t.created_at)}
                                  </div>
                                </div>
                              </div>
                            ))}
                ...
                <div className="member-info">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      {(['PENDING', 'APPROVED', 'RETURNED'].includes(review?.rating_status?.toUpperCase())) && hasApprovedSubtheme ? (
                        <div className="h-stack" style={{ gap: 8 }}>
                          <Badge cls={review?.rating_status?.toUpperCase() === 'APPROVED' ? 'green' : review?.rating_status?.toUpperCase() === 'RETURNED' ? 'yellow' : 'cyan'}>
                            {review?.rating_status?.toUpperCase() === 'APPROVED' ? 'Evidence Approved' : review?.rating_status?.toUpperCase() === 'RETURNED' ? 'Evidence Returned' : 'Evidence Submitted'}
                          </Badge>
                          <button className="badge badge-purple" onClick={() => {
                            setActivePanel(m.id);
                            setBinaryInputs({ 
                              themeResults: review?.theme_results || {}, 
                            ...
                            {review?.rating_status === 'APPROVED' ? 'View/Edit Rating →' : 'Enter Rating & Inputs →'}
                          </button>
                        </div>
                      ) : (
                        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                          {!hasApprovedSubtheme ? (
                            <Badge cls="gray" style={{ opacity: 0.6 }}>○ Themes pending validation</Badge>
                          ) : (
                            <Badge cls="gray" style={{ opacity: 0.6 }}>○ Awaiting evidence</Badge>
                          )}
                        </div>
                      )}
                    </div>
              ...
              {activePanel === m.id && (
                <div className="binary-panel">
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div style={{ flex: 1 }}>
                                ...
                                <div style={{ fontSize: 10, color: 'var(--purple)', fontWeight: 700, marginBottom: 4 }}>
                                  ALIGNED TO: {rootThemes.find(rt => rt.id === t.parent_id || rt.id === t.main_theme_id)?.title || "General Pillar"}
                                </div>
                  ...
                  <div className="single-input-row" style={{ marginTop: 24, padding: '20px', background: 'rgba(0,178,236,0.05)', borderRadius: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid var(--cyan)' }}>
                    ...
                    <div>
                      {(() => {
                        const vals = Object.values(binaryInputs.themeResults);
                        const yesCount = vals.filter(v => v === 'YES').length;
                        const neutralCount = vals.filter(v => v === 'NEUTRAL').length;
                        
                        if (yesCount >= 2) return <Badge cls="green" style={{ fontSize: 14, padding: '8px 16px' }}>OVERALL: EXCEEDED (YES)</Badge>;
                        if (yesCount + neutralCount >= 2) return <Badge cls="gray" style={{ fontSize: 14, padding: '8px 16px', color: 'var(--text1)' }}>OVERALL: DELIVERED (NEUTRAL)</Badge>;
            ...
            <tbody>
              {team.map((m, i) => {
                ...
                const getResult = (monthPrefix) => {
                  // Map current UUID cycle to the correct month column (April)
                  if (monthPrefix === 'APR') {
                    const r = memberReviews.find(rev => rev.cycle_id === CYCLE_ID || rev.cycle_id === 'APR_2026');
                    return r ? (r.overall_result === 'YES' ? 'Y' : 'N') : '—';
                  }

                  const r = memberReviews.find(rev => rev.cycle_id === `${monthPrefix}_2026`);
                  return r ? (r.overall_result === 'YES' ? 'Y' : 'N') : '—';
                };
...
function DirectorPortal({ profile, activeUser, showToast }) {
  ...
  return (
    <div className="page" style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      {/* 5. EXCEPTIONS - LIGHT MODE */}
      <div className="frame" style={{ background: '#fff' }}>
        <div className="sec-title" style={{ fontSize: 15 }}>Exception Reports</div>
        <table className="hist-table-v3" style={{ marginTop: 12 }}>
          <thead>
            <tr>
              <th style={{ fontSize: 10 }}>EXCEPTION TYPE</th>
              <th style={{ textAlign: 'center', fontSize: 10 }}>COUNT</th>
              <th style={{ fontSize: 10 }}>DEPARTMENT</th>
              <th style={{ fontSize: 10 }}>ACTION REQUIRED</th>
            </tr>
          </thead>
          <tbody>
            {[{ t: '3+ consecutive No results', c: 8, d: 'Sales, Operations', a: 'Manager review required' }, { t: 'Missing manager submission', c: 12, d: 'Marketing, Product', a: 'Escalate to department head' }, { t: 'Theme submission overdue', c: 19, d: 'All departments', a: 'Automated reminder sent' }].map((ex, i) => (
              <tr key={i}>
                <td style={{ fontSize: 12 }}>{ex.t}</td>
                <td style={{ textAlign: 'center' }}><div className="badge badge-red" style={{ padding: '2px 8px' }}>{ex.c}</div></td>
                <td style={{ fontSize: 12 }}>{ex.d}</td>
                <td style={{ fontSize: 12, color: 'var(--text3)' }}>{ex.a}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function App() {
    ...
    async function initSession() {
      ...
      if (!user) {
        ...
        if (profileData) {
          ...
          setPage(profileData.role === 'manager' || profileData.role === 'hr' || profileData.role === 'hod' ? "overview" : "employee");
        } else {
  ...
  const TABS = [
    ...
    ["reports", "◈ Reports"],
    profile?.role === 'hr' || profile?.role === 'hod' ? ["upload", "◈ Data Management"] : null,
        ...
        <div className="nav-right">
          <div className="nav-user">
            <div className="v-stack" style={{ alignItems: 'flex-end', marginRight: 12 }}>
              ...
              <div className="h-stack" style={{ gap: 6, opacity: 0.6, fontSize: 10 }}>
                <Badge cls="purple" style={{ fontSize: 9, padding: '2px 6px' }}>{profile?.function || 'General'}</Badge>
          ...
          <div className="v-stack" style={{ gap: 8 }}>
            {profiles.map(p => (
              <button 
                ...
                onClick={() => {
                  setProfile(p);
                  setPage(p.role === 'manager' || p.role === 'hod' || p.role === 'hr' ? "overview" : "employee");
                  setIsOpen(false);
    ...
    <div className="page">
      ...
      <div className="stats-grid" style={{ marginBottom: 32 }}>
        <div className="frame" style={{ flex: 1 }}>
          <div className="sec-title">Upload Employee Data</div>
          <input type="file" accept=".xlsx, .xls" onChange={handleFileUpload} disabled={uploading} style={{ marginBottom: 12 }} />
          <div style={{ fontSize: 11, color: 'var(--text3)' }}>Expected columns: EmployeeID, FirstName, LastName, JobTitle, Department, Role, EmploymentType, Function, SubFunction</div>
        </div>
        <div className="frame" style={{ flex: 1 }}>
          <div className="sec-title">Data Extraction</div>
          <div className="v-stack" style={{ gap: 10 }}>
            <button className="btn-outline" style={{ fontSize: 11, padding: '10px' }} onClick={async () => {
              const ws = XLSX.utils.json_to_sheet(reportData);
              const wb = XLSX.utils.book_new();
              XLSX.utils.book_append_sheet(wb, ws, "Authorized_Staff");
              XLSX.writeFile(wb, `${profile?.role?.toUpperCase()}_Staff_Export.xlsx`);
            }}>Download Staff Hierarchy (.xlsx)</button>
            <button className="btn-outline" style={{ fontSize: 11, padding: '10px' }} onClick={async () => {
              const authorizedIds = reportData.map(p => p.id);
              const filteredReviews = allReviews.filter(r => authorizedIds.includes(r.employee_id));
              const ws = XLSX.utils.json_to_sheet(filteredReviews);
              const wb = XLSX.utils.book_new();
              XLSX.utils.book_append_sheet(wb, ws, `Reviews_${selectedYear}`);
              XLSX.writeFile(wb, `${profile?.role?.toUpperCase()}_Review_History_${selectedYear}.xlsx`);
            }}>Download Performance History (.xlsx)</button>
          </div>
        </div>
      </div>
    </div>
...
function Reports({ profile, activeUser, showToast }) {
  ...
  useEffect(() => {
    fetchReports();
  }, [activeUser, profile, selectedYear]);
  ...
  return (
    <div className="page" style={{ paddingBottom: 100 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div className="h-stack" style={{ gap: 12 }}>
          <div style={{ fontSize: 12, fontWeight: 700 }}>SELECT YEAR:</div>
          <select 
            className="input" 
        ...
        <div className="v-stack" style={{ gap: 20 }}>
          <div className="report-table-frame">
            <table className="report-table">
              ...
              <tbody>
                {reportData.map(d => {
                  const review = allReviews.find(r => r.employee_id === d.id);
                  return (
                ...
                <tbody>
                  {allProfiles.filter(p => p.manager_id === selectedManagerId).map(m => (
                    <tr key={m.id} onClick={() => setSelectedPersonId(m.id)} style={{ cursor: 'pointer', background: selectedPersonId === m.id ? 'var(--bg2)' : '' }}>
                      ...
                      <td><Badge cls={allReviews.find(r => r.employee_id === m.id)?.overall_result === 'YES' ? 'green' : 'red'}>{allReviews.find(r => r.employee_id === m.id)?.overall_result || 'PENDING'}</Badge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function OverviewPage({ profile }) {
  ...
  async function fetchLiveMetrics() {
    ...
    // 2. Reviews for cycle
    const { data: cycleReviews } = await supabase.from('monthly_reviews').select('overall_result, employee_id').eq('cycle_id', CYCLE_ID);
    setSubmissionCount(cycleReviews?.length || 0);
    setYesOutcomeCount(cycleReviews?.filter(r => r.overall_result === 'YES').length || 0);
      ...
      <div className="overview-section">
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
          <div className="frame" style={{ flex: 1, margin: 0, backdropFilter: 'blur(10px)', background: 'rgba(255,255,255,0.7)' }}>
            <div className="sec-title">Department Rating Analysis (YES Rate)</div>
            {depts.map(d => (
